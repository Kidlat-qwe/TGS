...
router.get('/available-advisers', async (req, res) => {
  try {
    ...
    // Get all active teachers who are not already class advisers for the given school year
    const query = `
      ...
      AND NOT EXISTS (
        SELECT 1
        FROM class c
        WHERE c.class_adviser_id = u.user_id
        AND c.school_year_id = $1
      )
      ORDER BY u.lname, u.fname
    `;
    
    const result = await pool.query(query, [schoolYearIdNum]);
  ...
  try {
    ...
    let query = `
      SELECT cs.class_id, cs.subject_id, c.grade_level, c.section,
             s.subject_name as subject, sy.school_year, sy.school_year_id
      FROM class_subject cs
      JOIN class c ON cs.class_id = c.class_id
      JOIN subject s ON cs.subject_id = s.subject_id
      JOIN school_year sy ON c.school_year_id = sy.school_year_id
